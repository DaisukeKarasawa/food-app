...
今回のプロダクトは、フロントと繋いでいないので、戻り値に何を返せば良いのかが感覚的に理解することができなかった。特に、エラーをどのように返して、フロントでどのような処理をするかをイメージすることができなかったため、ハンドリングやエラー時の戻り値が適切でない可能性が高い。それ以前に、エラーへの知識があまりないので、現時点で自分の望んだ場所でエラーメッセージをレスポンスすることはできているが、書き方が正しいのかを明確に理解できていない。
